...
def pictureDisplay():
    for i in range(4200):   #run for ~10 minutes?
        sleepTime = 0.5 # sleep time in seconds
        # Get picture message
...
# ----------------------------------------------------------------------
def main():
    cameraProcess = Process(target=pictureDisplay) # may need to add ", args=()" inside parenthesis
    sensorsProcess = Process(target=updateLEDs) # may need to add ", args=()" inside parenthesis
    cameraProcess.start()
